...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 3 :
        ...
        if sys.argv[2] == 'Y':
            ...
            DB.drop_table(cursor,table_name="embeddings")
            DB.create_table(cursor,table_name="embeddings")
    ...
    for chunk in chunks:
        ...
        DB.insert_embedding(text,source, embedding_vector, page_number, detected_language,cursor,table_name="embeddings")
        
        # Ottieni l'id del record appena inserito
        record_id = cursor.fetchone()[0]
        
        # Aggiorna il campo tsv_content solo per il record appena inserito
        DB.set_tsv(cursor,detected_language,record_id,table_name="embeddings")
